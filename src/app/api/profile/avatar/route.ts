import { NextRequest, NextResponse } from 'next/server';
import { getApiSession, unauthorizedResponse } from '@/lib/auth/utils';
import { saveAvatarPhoto, StorageError } from '@/lib/storage/local-fs';
import { UploadAvatarSchema } from '@/types/api';

export const dynamic = 'force-dynamic';

/**
 * POST /api/profile/avatar — upload foto profil pengguna login.
 * Mengembalikan URL foto; client lalu memanggil authClient.updateUser({image})
 * agar session Better Auth ikut diperbarui.
 */
export async function POST(req: NextRequest) {
  try {
    const session = await getApiSession(req);
    if (!session) return unauthorizedResponse();

    const body = await req.json();
    const parsed = UploadAvatarSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json(
        {
          code: 'VALIDATION_ERROR',
          message: 'Foto tidak valid',
          details: parsed.error.flatten(),
          timestamp: new Date().toISOString(),
        },
        { status: 400 }
      );
    }

    const url = await saveAvatarPhoto(parsed.data.photoBase64);
    return NextResponse.json({ url }, { status: 201 });
  } catch (error) {
    if (error instanceof StorageError) {
      return NextResponse.json(
        { code: `PHOTO_${error.code}`, message: error.message, timestamp: new Date().toISOString() },
        { status: 400 }
      );
    }
    console.error('POST /api/profile/avatar error:', error);
    return NextResponse.json(
      { code: 'INTERNAL_ERROR', message: 'Terjadi kesalahan sistem', timestamp: new Date().toISOString() },
      { status: 500 }
    );
  }
}
